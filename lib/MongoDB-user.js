  ...
  if(tokens.refresh_token && tokens.access_token){
    ...
    if(tokens.access_token){
      ...
      basic.updateOneData(database,'Users',{
        ...
        "APIs.YouTube.access_token" : access_token
      },function(err, results){
        if (results.result.n!==1){
          callback("Error adding refreshed token, user not found");
        }else{
          // console.log(results )
          callback(undefined, results);
        }
  ...
  basic.findData(database,'Users',{
    ...
    if (err){
      ...
      if (result.length===0){
        ...
        if(result[0].APIs.YouTube.refresh_token && result[0].APIs.YouTube.access_token){
...
function removeYouTubeAPI(username, callback){
  basic.updateOneData(database,'Users',{
      "user.username" : username
    },{
      "APIs.YouTube" : {
...
exports.removeYouTubeAPI = removeYouTubeAPI;
