      ...
      if (err) callback("hashing error!");
        ...
        basic.findData(database,"Users",{"user.username":username},function(result){
          if (result.length===0){
            ...
            function(err, result){
              if (err) {
                // print verbose information in server logs, but give a simple error message to user
                console.log(err);
                callback(err);
              } else {
    ...
    if (result.length===0){
      ...
      bcrypt.compare(password,result[0].user.password,function(err,res){
        if (res===true){
          ...
          callback(false,"Passwords do not match. Please contact a system administrator.");
        }
      });
    }
  });
